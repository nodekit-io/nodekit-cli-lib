...
function updateSubprojectHelp() {
    console.log('Updates the subproject path of the NodeKitLib entry to point to this script\'s version of NodeKit.');
    console.log('Usage: NodeKitVersion/bin/update_nodekit_project path/to/your/app.xcodeproj [path/to/NodeKitLib.xcodeproj]');
}
...
function copyJsAndNodeKitLib(projectPath, projectName, use_shared) {
    shell.cp('-f', path.join(ROOT, 'NodeKitLib', 'nodekit.js'), path.join(projectPath, 'www'));
    shell.cp('-rf', path.join(ROOT, 'nodekit-js-src'), path.join(projectPath, 'platform_www'));
    shell.cp('-f', path.join(ROOT, 'NodeKitLib', 'nodekit.js'), path.join(projectPath, 'platform_www'));

    fs.lstat(path.join(projectPath, 'NodeKitLib'), function(err, stats) {
        if (!err) {
            if (stats.isSymbolicLink()) {
                fs.unlinkSync(path.join(projectPath, 'NodeKitLib'));
            } else {
                shell.rm('-rf', path.join(projectPath, 'NodeKitLib'));
            }
        }

        if (use_shared) {
            update_nodekit_subproject([path.join(projectPath, projectName +'.xcodeproj', 'project.pbxproj')]);
            // Symlink not used in project file, but is currently required for plugman because
            // it reads the VERSION file from it (instead of using the nodekit/version script
            // like it should).
            fs.symlinkSync(path.join(ROOT, 'NodeKitLib'), path.join(projectPath, 'NodeKitLib'));
        } else {
            var r = path.join(projectPath, projectName);
            shell.mkdir('-p', path.join(projectPath, 'NodeKitLib', 'NodeKitLib.xcodeproj'));
            shell.cp('-f', path.join(r, '.gitignore'), projectPath);
            shell.cp('-rf',path.join(ROOT, 'NodeKitLib', 'Classes'), path.join(projectPath, 'NodeKitLib'));
            shell.cp('-f', path.join(ROOT, 'NodeKitLib', 'VERSION'), path.join(projectPath, 'NodeKitLib'));
            shell.cp('-f', path.join(ROOT, 'NodeKitLib', 'nodekit.js'), path.join(projectPath, 'NodeKitLib'));
            shell.cp('-f', path.join(ROOT, 'NodeKitLib', 'NodeKitLib_Prefix.pch'), path.join(projectPath, 'NodeKitLib'));
            shell.cp('-f', path.join(ROOT, 'NodeKitLib', 'NodeKitLib.xcodeproj', 'project.pbxproj'), path.join(projectPath, 'NodeKitLib', 'NodeKitLib.xcodeproj'));
            update_nodekit_subproject([path.join(r+'.xcodeproj', 'project.pbxproj'), path.join(projectPath, 'NodeKitLib', 'NodeKitLib.xcodeproj', 'project.pbxproj')]);
        }
    });
}

function copyScripts(projectPath) {
    var srcScriptsDir = path.join(ROOT, 'bin', 'templates', 'scripts', 'nodekit');
 ...
 * @param {String} project_template_dir path to nodekit-ios template directory
 ...
 *
 * - --link (optional): Link directly against the shared copy of the NodeKitLib instead of a copy of it
 * - --cli (optional): Use the CLI-project template
 * - <path_to_new_project>: Path to your new NodeKit iOS project
...
exports.createProject = function(project_path, package_name, project_name, opts) {
    ...
    events.emit('log', 'Creating NodeKit project for the iOS platform:');
    ...
    //NodeKitLib stuff
    copyJsAndNodeKitLib(project_path, project_name, use_shared);
...
exports.updateProject = function(projectPath, opts) {
    ...
    setShellFatal(true, function() {
        ...
        copyJsAndNodeKitLib(projectPath, projectName, opts.link);
...
function update_nodekit_subproject(argv) {
    if (argv.length < 1 || argv.length > 2)
    {
        updateSubprojectHelp();
        throw new Error('Usage error for update_nodekit_subproject');
    }

    var projectPath = AbsProjectPath(argv[0]),
        nodekitLibXcodePath;
    if (argv.length < 2) {
        nodekitLibXcodePath = path.join(ROOT, 'NodeKitLib', 'NodeKitLib.xcodeproj');
    }
    else {
        nodekitLibXcodePath = AbsProjectPath(argv[1]);
    }

    var parentProjectPath = AbsParentPath(projectPath),
        subprojectPath = relpath(nodekitLibXcodePath, parentProjectPath),
        REGEX = /(.+PBXFileReference.+wrapper.pb-project.+)(path = .+?;)(.*)(sourceTree.+;)(.+)/,
        newLine,
        lines = shell.grep('NodeKitLib.xcodeproj', path.join(projectPath, 'project.pbxproj')),
        found = false;
    ...
    for (var i = 0; i < lines.length; ++i) {
        if (lines[i].match(REGEX)) {
            ...
            if (!newLine.match('name')) {
                newLine = newLine.replace('path = ', 'name = NodeKitLib.xcodeproj; path = ');
            }
...
exports.update_nodekit_subproject = update_nodekit_subproject;
