...
function tryCommand(cmd, errMsg, catchStderr) {
    ...
    child_process.exec(cmd, function(err, stdout, stderr) {
        if (err) d.reject(new NodeKitError(errMsg));
...
module.exports.check_gradle = function() {
    ...
    if (!sdkDir)
        return Q.reject(new NodeKitError('Could not find gradle wrapper within Android SDK. Could not find Android SDK directory.\n' +
            'Might need to install Android SDK or set up \'ANDROID_HOME\' env variable.'));

    var wrapperDir = path.join(sdkDir, 'tools', 'templates', 'gradle', 'wrapper');
    if (!fs.existsSync(wrapperDir)) {
        return Q.reject(new NodeKitError('Could not find gradle wrapper within Android SDK. Might need to update your Android SDK.\n' +
    ...
    return Q().then(function() {
        ...
        if (androidCmdPath && !hasAndroidHome) {
            ...
            if (path.basename(parentDir) == 'tools') {
                ...
                throw new NodeKitError('Failed to find \'ANDROID_HOME\' environment variable. Try setting setting it manually.\n' +
        ...
        if (!fs.existsSync(process.env['ANDROID_HOME'])) {
            throw new NodeKitError('\'ANDROID_HOME\' environment variable is set to non-existent path: ' + process.env['ANDROID_HOME'] +
...
module.exports.run = function() {
     ...
     .then(function(values) {
         ...
         if (!values[0]) {
            throw new NodeKitError('Requirements check failed for JDK 1.8 or greater');
         }


         if (!values[1]) {
            throw new NodeKitError('Requirements check failed for Android SDK');
         }
     });
};
 ...
 * as an array of Requirement objects. This method intended to be used by nodekit-lib check_reqs method
 *
