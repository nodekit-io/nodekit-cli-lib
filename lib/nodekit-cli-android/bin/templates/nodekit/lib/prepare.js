...
module.exports.prepare = function (nodekitProject, options) {
    var self = this;
    var platformResourcesDir = path.relative(nodekitProject.root, path.join(this.locations.root, 'res'));

    var platformJson = PlatformJson.load(this.locations.root, this.platform);
    var munger = new PlatformMunger(this.platform, this.locations.root, platformJson, new PluginInfoProvider());

    this._config = updateConfigFilesFrom(nodekitProject.projectConfig, munger, this.locations);
    ...
    .then(function () {
        updateIcons(nodekitProject, platformResourcesDir);
        updateSplashes(nodekitProject, platformResourcesDir);
    })
    .then(function () {
...
function updateWww(nodekitProject, destinations) {
    ...
    var merges_path = path.join(nodekitProject.root, 'merges', 'android');
...
function updateProjectAccordingTo(platformConfig, locations) {
    ...
    var java_files = shell.ls(javaPattern).filter(function(f) {
        return shell.grep(/extends\s+NodeKitActivity/g, f);
    });

    if (java_files.length === 0) {
        throw new NodeKitError('No Java files found that extend NodeKitActivity.');
    } else if(java_files.length > 1) {
        events.emit('log', 'Multiple candidate Java files that extend NodeKitActivity found. Guessing at the first one, ' + java_files[0]);
    }
...
function updateSplashes(nodekitProject, platformResourcesDir) {
    var resources = nodekitProject.projectConfig.getSplashScreens('android');
    ...
    var resourceMap = mapImageResources(nodekitProject.root, platformResourcesDir, 'screen.png');
...
function updateIcons(nodekitProject, platformResourcesDir) {
    var icons = nodekitProject.projectConfig.getIcons('android');
    ...
    var resourceMap = mapImageResources(nodekitProject.root, platformResourcesDir, 'icon.png');
