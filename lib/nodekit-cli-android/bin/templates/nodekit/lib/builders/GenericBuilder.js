...
GenericBuilder.prototype.readProjectProperties = function () {
    ...
    return {
        ...
        gradleIncludes: findAllUniq(data, /^\s*nodekit\.gradle\.include\.\d+=(.*)(?:\s|$)/mg),
        systemLibs: findAllUniq(data, /^\s*nodekit\.system\.library\.\d+=(.*)(?:\s|$)/mg)
    };
};
...
GenericBuilder.prototype.extractRealProjectNameFromManifest = function () {
    ...
    if (!m) {
        throw new NodeKitError('Could not find package name in ' + manifestPath);
    }
