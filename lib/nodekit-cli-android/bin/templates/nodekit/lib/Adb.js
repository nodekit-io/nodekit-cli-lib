...
Adb.install = function (target, packagePath, opts) {
    ...
    .then(function(output) {
        ...
        if (output.match(/Failure/)) {
            ...
            return Q.reject(new NodeKitError('Failed to install apk to device: ' + output));
        }
    });
};
...
Adb.shell = function (target, shellCommand) {
    ...
    .catch(function (output) {
        return Q.reject(new NodeKitError('Failed to execute shell command "' +
...
Adb.start = function (target, activityName) {
    ...
    .catch(function (output) {
        return Q.reject(new NodeKitError('Failed to start application "' +
