...
function copyJsAndNodeKitLib(projectPath, projectName, use_shared) {
    ...
    fs.lstat(path.join(projectPath, 'NodeKitLib'), function(err, stats) {
        ...
        if (use_shared) {
            ...
            var srcLib = path.join(ROOT, 'NodeKitLib');
            var dstLib = path.join(projectPath, 'NodeKitLib');
            shell.mkdir('-p', path.join(dstLib, 'NodeKitLib.xcodeproj'));
            shell.cp('-f', path.join(r, '.gitignore'), projectPath);
            shell.cp('-rf',path.join(srcLib, 'NodeKitLib'), dstLib);
            shell.cp('-f', path.join(srcLib, 'VERSION'), dstLib);
            shell.cp('-f', path.join(srcLib, 'nodekit.js'), dstLib);
            shell.cp('-f', path.join(srcLib, 'NodeKitLib', 'NodeKitLib-Prefix.pch'), path.join(dstLib, 'NodeKitLib'));
            shell.cp('-f', path.join(srcLib, 'NodeKitLib.xcodeproj', 'project.pbxproj'), path.join(dstLib, 'NodeKitLib.xcodeproj'));
            update_nodekit_subproject([path.join(r + '.xcodeproj', 'project.pbxproj'), path.join(dstLib, 'NodeKitLib.xcodeproj', 'project.pbxproj')]);
        }
    });
}
 ...
 * - <path_to_new_project>: Path to your new NodeKit OSX project
