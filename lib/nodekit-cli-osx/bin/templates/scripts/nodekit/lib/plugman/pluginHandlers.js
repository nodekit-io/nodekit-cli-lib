...
// These frameworks are required by nodekit-osx by default. We should never add/remove them.
...
var handlers = {
    ...
    'resource-file':{
        install:function(obj, plugin, project, options) {
            ...
            if (!fs.existsSync(srcFile)) throw new NodeKitError('cannot find "' + srcFile + '" osx <resource-file>');
            if (fs.existsSync(destFile)) throw new NodeKitError('target destination "' + destFile + '" already exists');
    ...
    'framework':{ // CB-5238 custom frameworks only
        install:function(obj, plugin, project, options) {
            ...
            if (!fs.existsSync(srcFile)) throw new NodeKitError('cannot find "' + srcFile + '" osx <framework>');
            if (fs.existsSync(targetDir)) throw new NodeKitError('target destination "' + targetDir + '" already exists');
    ...
    'asset':{
        install:function(obj, plugin, project, options) {
            if (!obj.src) {
                throw new NodeKitError('<asset> tag without required "src" attribute. plugin=' + plugin.dir);
            }
            if (!obj.target) {
                throw new NodeKitError('<asset> tag without required "target" attribute');
            }
...
function copyFile (plugin_dir, src, project_dir, dest, link) {
    ...
    if (real_path.indexOf(real_plugin_path) !== 0)
        throw new NodeKitError('"' + src + '" not located within plugin!');

    dest = path.resolve(project_dir, dest);

    // check that dest path is located in project directory
    if (dest.indexOf(project_dir) !== 0)
        throw new NodeKitError('"' + dest + '" not located within project!');
