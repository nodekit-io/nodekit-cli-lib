...
module.exports = function(target, dest, opts) {
    ...
    .then(function() {
        ...
        } else return Q.reject(new NodeKitError('Need to supply a target and destination'));
    ...
    .fail(function(err){
        return Q.reject(new NodeKitError(err));
    });
};
 ...
 * nodekit-fetch will use trimID in that case.
 *
 ...
 *
 * @param {String} target    target that was passed into nodekit-fetch.
...
function getPath(id, dest) {
    ...
    if(fs.existsSync(finalDest)){
        return finalDest;
    } else return Q.reject(new NodeKitError('Failed to get absolute path to installed module'));
}

/*
 * Checks to see if npm is installed on the users system
 * @return {Promise|Error} Returns true or a nodekit error.
 */

function isNpmInstalled() {
    if(!shell.which('npm')) {
        return Q.reject(new NodeKitError('"npm" command line tool is not installed: make sure it is accessible on your PATH.'));
    }
    return Q();
}
