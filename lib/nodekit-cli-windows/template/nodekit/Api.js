...
function Api(platform, platformRootDir, eventEmitter) {
    ...
    this.locations = {
        ...
        defaultConfigXml: path.join(self.root, 'nodekit/defaults.xml'),
        // NOTE: Due to platformApi spec we need to return relative paths here
        nodekitJs: 'template/www/nodekit.js',
...
Api.createPlatform = function (destinationDir, projectConfig, options, events) {
    ...
    try {
        ...
        .then(function () {
            var PlatformApi = require(path.resolve(destinationDir, 'nodekit/Api'));
 ...
 */
Api.prototype.prepare = function (nodekitProject, prepareOptions) {
    return require('./lib/prepare').prepare.call(this, nodekitProject, prepareOptions);
};
...
Api.prototype.removePlugin = function (plugin, uninstallOptions) {
    ...
    return pluginManager
        ...
        .then(function () {
            // CB-11657 Add BOM to nodekit_plugins, since it is was
            // regenerated after plugin uninstallation and does not have BOM
            return require('./lib/prepare').addBOMToFile(path.resolve(self.locations.www, 'nodekit_plugins.js'));
        })
        // CB-11022 return non-falsy value to indicate
        // that there is no need to run prepare after
        .thenResolve(true);
