...
var projFiles = {
    phone: 'NodeKitApp.Phone.jsproj',
    win: 'NodeKitApp.Windows.jsproj',
    win10: 'NodeKitApp.Windows10.jsproj'
};
var projFilesToManifests = {
    'NodeKitApp.Phone.jsproj': 'package.phone.appxmanifest',
    'NodeKitApp.Windows.jsproj': 'package.windows.appxmanifest',
    'NodeKitApp.Windows10.jsproj': 'package.windows10.appxmanifest'
};

var ROOT = path.resolve(__dirname, '../..');

// builds nodekit-windows application with parameters provided.
// See 'help' function for args list
module.exports.run = function run (buildOptions) {

    ROOT = this.root || ROOT;

    if (!utils.isNodeKitProject(this.root)){
        return Q.reject(new NodeKitError('Could not find project at ' + this.root));
    }
...
module.exports.getBuildTargets  = function(isWinSwitch, isPhoneSwitch, projOverride) {
    ...
    if (isWinSwitch || noSwitches) { // if --win or no arg
        ...
        switch(windowsTargetVersion.toLowerCase()) {
            case '8':
            case '8.0':
                throw new NodeKitError('windows8 platform is deprecated. To use windows-target-version=8.0 you must downgrade to nodekit-windows@4.');
            case '8.1':
            ...
            case 'uap':
                ...
                break;
            default:
                throw new NodeKitError('Unsupported windows-target-version value: ' + windowsTargetVersion);
        }
    }
...
function parseAndValidateArgs(options) {
    ...
    if (options.debug && options.release) {
        throw new NodeKitError('Cannot specify "debug" and "release" options together.');
    }

    if (args.phone && args.win) {
        throw new NodeKitError('Cannot specify "phone" and "win" options together.');
    }
...
function parseBuildConfig(buildConfigPath, buildType) {
    ...
    try {
        ...
        if (e.code === 'ENOENT') {
            throw new NodeKitError('Specified build config file does not exist: ' + buildConfigPath);
        }
        else {
            throw e;
        }
    ...
    if(windowsInfo.publisherId) {
        ...
        if (!publisherRegex.test(windowsInfo.publisherId)) {
            throw new NodeKitError('Invalid publisher id: ' + windowsInfo.publisherId);
        }
...
function buildTargets(allMsBuildVersions, config) {
    ...
    var msbuild = getLatestMSBuild(allMsBuildVersions);
    if (!msbuild) {
        return Q.reject(new NodeKitError('No valid MSBuild was detected for the selected target.'));
    }
    ...
    var buildsCompleted = buildConfigs.reduce(function (promise, build, index, configsArray) {
         return promise.then(function () {
            ...
            var otherProperties = { };
            // Only add the NodeKitBundlePlatforms argument when on the last build step
            if (shouldBundle && index === configsArray.length - 1) {
                otherProperties.NodeKitBundlePlatforms = bundleTerms;
            } else if (shouldBundle) {
                otherProperties.NodeKitBundlePlatforms = build.arch;
            }
...
function clearIntermediatesAndGetPackage(bundleTerms, config, hasAnyCpu) {
    ...
    // However, that generates intermediate bundles, like "NodeKitApp.Windows10_0.0.1.0_x64.appxbundle"
    // We need to clear the intermediate bundles, or else "nodekit run" will fail because of too
