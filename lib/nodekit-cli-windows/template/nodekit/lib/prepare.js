...
var DEFAULT_DESCRIPTION = 'NodeKitApp';

var PROJECT_WINDOWS10   = 'NodeKitApp.Windows10.jsproj',
...
function applyCoreProperties(config, manifest) {
    ...
    // NodeKit apps.  So, we have to create a Windows-specific preference here.
    ...
    // strings which are typically used in NodeKit aren't valid for Store ingestion.
    // Here, we check for Windows-specific preferences, and if we find it, prefer that over
    // the NodeKit <widget> areas.
...
function copyImages(nodekitProject, locations) {
    var images = nodekitProject.projectConfig.getIcons('windows')
        .concat(nodekitProject.projectConfig.getSplashScreens('windows'));
    ...
    var imagesDir = path.join(path.relative(nodekitProject.root, locations.root), 'images');
...
module.exports.prepare = function (nodekitProject, options) {
    ...
    .then(function () {
        copyImages(nodekitProject, self.locations);
...
function updateWww(nodekitProject, destinations) {
    ...
    var merges_path = path.join(nodekitProject.root, 'merges', 'windows');
