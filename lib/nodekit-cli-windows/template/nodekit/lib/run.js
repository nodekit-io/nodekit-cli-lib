...
module.exports.run = function (options) {
    ...
    if (ranWithElevatedPermissions())
        return Q.reject(new NodeKitError('Can not run this platform with administrative ' +
    ...
    if (options.debug && options.release) {
        return Q.reject(new NodeKitError('Only one of "debug"/"release" options should be specified'));
    }
    if ((options.device && options.emulator) || ((options.device || options.emulator) && options.target)) {
        return Q.reject(new NodeKitError('Only one of "device"/"emulator"/"target" options should be specified'));
    }
    if (args.phone && args.win) {
        return Q.reject(new NodeKitError('Only one of "phone"/"win" options should be specified'));
    }

    // Get build/deploy options
    var buildType    = options.release ? 'release' : 'debug';
    // CB-11478 Allow to specify 'archs' parameter as either cli or platform
    // option i.e. 'nodekit run --archs' vs. 'nodekit run -- --archs'
    ...
    var deployTarget = options.target ? options.target : (options.emulator ? 'emulator' : 'device');
     ...
     if (!buildTargets || buildTargets.length <= 0) {
         return Q.reject(new NodeKitError('Unable to determine deploy target.'));
     }
...
// @param   {String}  projFile Project file, for example 'NodeKitApp.Windows10.jsproj'
// @returns {String}  Proejct type, for example 'windows10'
function projFileToType(projFile)
{
    return projFile.replace(/NodeKitApp|jsproj|\./gi, '').toLowerCase();
}
