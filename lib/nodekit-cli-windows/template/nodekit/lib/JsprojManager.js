...
var xml_helpers = require('nodekit-cli-lib')['nodekit-common'].xmlHelpers;
var AppxManifest = require('./AppxManifest');
...
jsprojManager.getProject = function (directory) {
    ...
    if (projectFiles.length === 0) {
        throw (new NodeKitError('The directory ' + directory +
...
jsprojManager.prototype = {
    ...
    addProjectReference: function (relative_path, targetConditions) {
        ...
        if (!projectTypeGuid) {
            throw new NodeKitError("Unrecognized project type at " + relative_path + " (not .csproj or .vcxproj)");
        }
        ...
        getSolutionPaths(this.projectFolder).forEach(function (solutionPath) {
            ...
            if (useProjItems) {
                ...
                if (!jsProjectFound) {
                    throw new NodeKitError("No jsproj found in solution");
                }
            } else {
        ...
        // Add the ItemGroup/ProjectReference to each matching nodekit project :
