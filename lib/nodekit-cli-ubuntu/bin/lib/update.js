...
exports.updateProject = function(projectPath) {
    ...
    shell.cp(path.join(ROOT, 'www', 'nodekit.js'), path.join(projectPath, 'www'));

    shell.rm('-r', path.join(projectPath, 'nodekit'));
    ...
    shell.cp('-r', path.join(ROOT, 'bin/check_reqs'), path.join(projectPath, 'nodekit'));
    shell.cp('-r', path.join(ROOT, 'bin', 'lib', 'check_reqs.js'), path.join(projectPath, 'nodekit', 'lib'));

    // Copy the default template including the defaults.xml for the Ubuntu platform.
    shell.cp('-r', path.join(ROOT, 'bin', 'templates', 'project', 'nodekit'), projectPath);
