...
var PluginManager = require('nodekit-cli-lib')['nodekit-common'].PluginManager;
var Q = require('q');
...
function setupEvents(externalEventEmitter) {
    if (externalEventEmitter) {
        ...
        NodeKitLogger.get().subscribe(events);
    }
}
...
function Api(platform, platformRootDir, events) {
    ...
    var xcodeProjDir;
    var xcodeNodeKitProj;

    try {
        xcodeProjDir = fs.readdirSync(this.root).filter( function(e) { return e.match(/\.xcodeproj$/i); })[0];
        if (!xcodeProjDir) {
            throw new NodeKitError('The provided path "' + this.root + '" is not a NodeKit iOS project.');
        }

        var nodekitProjName = xcodeProjDir.substring(xcodeProjDir.lastIndexOf(path.sep)+1, xcodeProjDir.indexOf('.xcodeproj'));
        xcodeNodeKitProj = path.join(this.root, nodekitProjName);
    } catch(e) {
        throw new NodeKitError('The provided path "'+this.root+'" is not a NodeKit iOS project.');
    }
...
Api.createPlatform = function (destination, config, options, events) {
    ...
    try {
        ...
        .then(function () {
            ...
            var PlatformApi = require(path.resolve(destination, 'nodekit/Api'));
 ...
 */
Api.prototype.prepare = function (nodekitProject) {
    return require('./lib/prepare').prepare.call(this, nodekitProject);
};
...
Api.prototype.addPlugin = function (plugin, installOptions) {
    ...
    return PluginManager.get(self.platform, self.locations, xcodeproj)
        ...
        .then(function(frameworkPods) {
            ...
            var project_name = self.locations.xcodeNodeKitProj.split('/').pop();
