...
module.exports.prepare = function (nodekitProject, options) {
    ...
    .then(function () {
        ...
        updateLaunchStoryboardImages(nodekitProject, self.locations);
    })
    .then(function () {
...
function updateProject(platformConfig, locations) {
    ...
    return handleBuildSettings(platformConfig, locations, infoPlist).then(function() {
        if (name == originalName) {
            ...
            'Make sure you save your plugins beforehand using `nodekit plugin save`.\n' +
            '\tnodekit plugin save\n' +
            '\tnodekit platform rm ios\n' +
            '\tnodekit platform add ios\n'
            ;

            return Q.reject(new NodeKitError(errorString));
        }
    });
}
 ...
 * 
 * @param  {Object} nodekitProject     The nodekit project
 * @param  {Object} locations          A dictionary containing useful location paths
 */
function updateLaunchStoryboardImages(nodekitProject, locations) {
    var splashScreens = nodekitProject.projectConfig.getSplashScreens('ios');
    var platformProjDir = path.relative(nodekitProject.root, locations.xcodeNodeKitProj);
    var launchStoryboardImagesDir = getLaunchStoryboardImagesDir(nodekitProject.root, platformProjDir);
    ...
    if (launchStoryboardImagesDir) {
        ...
        fs.writeFileSync(path.join(nodekitProject.root, launchStoryboardImagesDir, 'Contents.json'),
