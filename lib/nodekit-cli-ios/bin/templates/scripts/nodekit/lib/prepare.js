...
var FileUpdater = require('nodekit-cli-lib')['nodekit-common'].FileUpdater;

/*jshint sub:true*/

module.exports.prepare = function (nodekitProject, options) {
    var self = this;

    this._config = updateConfigFile(nodekitProject.projectConfig,
        configMunger.get(this.locations.root), this.locations);

    // Update own www dir with project's www assets and plugins' assets and js-files
    return Q.when(updateWww(nodekitProject, this.locations))
    .then(function () {
        // update project according to config.xml changes.
        return updateProject(self._config, self.locations);
    })
    .then(function () {
        updateIcons(nodekitProject, self.locations);
        updateSplashScreens(nodekitProject, self.locations);
    })
    .then(function () {
 ...
 *
 * @param   {Object}  nodekitProject   An object which describes nodekit project.
 * @param   {boolean} destinations     An object that contains destinations
 *   paths for www files.
 */
function updateWww(nodekitProject, destinations) {
    var sourceDirs = [
        path.relative(nodekitProject.root, nodekitProject.locations.www),
        path.relative(nodekitProject.root, destinations.platformWww)
    ];

    // If project contains 'merges' for our platform, use them as another overrides
    var merges_path = path.join(nodekitProject.root, 'merges', 'ios');
    ...
    var targetDir = path.relative(nodekitProject.root, destinations.www);
    events.emit(
        'verbose', 'Merging and updating files from [' + sourceDirs.join(', ') + '] to ' + targetDir);
    FileUpdater.mergeAndUpdateDir(
        sourceDirs, targetDir, { rootDir: nodekitProject.root }, logFileOp);
}
...
function updateProject(platformConfig, locations) {
    ...
    var plistFile = path.join(locations.xcodeNodeKitProj, originalName + '-Info.plist');
    ...
    return handleBuildSettings(platformConfig, locations).then(function() {
        ...
        try {
            ...
            return Q.reject(new NodeKitError('Could not parse project.pbxproj: ' + err));
        }
        ...
        shell.mv(path.join(locations.xcodeNodeKitProj, originalName + '-Info.plist'), path.join(locations.xcodeNodeKitProj, name + '-Info.plist'));
        shell.mv(path.join(locations.xcodeNodeKitProj, originalName + '-Prefix.pch'), path.join(locations.xcodeNodeKitProj, name + '-Prefix.pch'));
        ...
        shell.mv(locations.xcodeNodeKitProj, path.join(locations.root, name));

        // Update locations with new paths
        locations.xcodeNodeKitProj = path.join(locations.root, name);
        locations.configXml = path.join(locations.xcodeNodeKitProj, 'config.xml');
...
function updateIcons(nodekitProject, locations) {
    var icons = nodekitProject.projectConfig.getIcons('ios');
    ...
    var platformProjDir = path.relative(nodekitProject.root, locations.xcodeNodeKitProj);
    var iconsDir = getIconsDir(nodekitProject.root, platformProjDir);
    var resourceMap = mapIconResources(icons, iconsDir);
    events.emit('verbose', 'Updating icons at ' + iconsDir);
    FileUpdater.updatePaths(
        resourceMap, { rootDir: nodekitProject.root }, logFileOp);
}

function cleanIcons(projectRoot, projectConfig, locations) {
    var icons = projectConfig.getIcons('android');
    if (icons.length > 0) {
        var platformProjDir = path.relative(projectRoot, locations.xcodeNodeKitProj);
...
function updateSplashScreens(nodekitProject, locations) {
    var splashScreens = nodekitProject.projectConfig.getSplashScreens('ios');
    ...
    var platformProjDir = path.relative(nodekitProject.root, locations.xcodeNodeKitProj);
    var splashScreensDir = getSplashScreensDir(nodekitProject.root, platformProjDir);
