...
function Podfile(podFilePath, projectName) {
    ...
    if (filename !== Podfile.FILENAME) {
        throw new NodeKitError(util.format('Podfile: The file at %s is not `%s`.', this.path, Podfile.FILENAME));
    }

    if (!projectName) {
        throw new NodeKitError('Podfile: The projectName was not specified in the constructor.');
    }
...
Podfile.prototype.getTemplate = function() {
    return util.format(
            '# DO NOT MODIFY -- auto-generated by Apache NodeKit\n' +
            'platform :ios, \'8.0\'\n' +
            'target \'%s\' do\n' +
            '\tproject \'%s.xcodeproj\'\n' +
            '%s\n' +
...
Podfile.prototype.addSpec = function(name, spec) {
    ...
    if (!name.length) { // blank names are not allowed
        throw new NodeKitError('Podfile addSpec: name is not specified.');
    }
