...
- (void)updateSettings:(NSDictionary*)settings
{
    ...
    uiWebView.scalesPageToFit = [settings nodekitBoolSettingForKey:@"EnableViewportScale" defaultValue:NO];
    uiWebView.allowsInlineMediaPlayback = [settings nodekitBoolSettingForKey:@"AllowInlineMediaPlayback" defaultValue:NO];
    uiWebView.mediaPlaybackRequiresUserAction = [settings nodekitBoolSettingForKey:@"MediaPlaybackRequiresUserAction" defaultValue:YES];
    uiWebView.mediaPlaybackAllowsAirPlay = [settings nodekitBoolSettingForKey:@"MediaPlaybackAllowsAirPlay" defaultValue:YES];
    uiWebView.keyboardDisplayRequiresUserAction = [settings nodekitBoolSettingForKey:@"KeyboardDisplayRequiresUserAction" defaultValue:YES];
    uiWebView.suppressesIncrementalRendering = [settings nodekitBoolSettingForKey:@"SuppressesIncrementalRendering" defaultValue:NO];
    uiWebView.gapBetweenPages = [settings nodekitFloatSettingForKey:@"GapBetweenPages" defaultValue:0.0];
    uiWebView.pageLength = [settings nodekitFloatSettingForKey:@"PageLength" defaultValue:0.0];

    id prefObj = nil;

    // By default, DisallowOverscroll is false (thus bounce is allowed)
    BOOL bounceAllowed = !([settings nodekitBoolSettingForKey:@"DisallowOverscroll" defaultValue:NO]);
    ...
    NSString* decelerationSetting = [settings nodekitSettingForKey:@"UIWebViewDecelerationSpeed"];
    ...
    prefObj = [settings nodekitSettingForKey:@"PaginationBreakingMode"];
    if (prefObj != nil) {
    ...
    prefObj = [settings nodekitSettingForKey:@"PaginationMode"];
    if (prefObj != nil) {
