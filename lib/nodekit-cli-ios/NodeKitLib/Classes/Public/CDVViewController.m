...
- (void)printVersion
{
    NSLog(@"NodeKit native platform version %@ is starting.", CDV_VERSION);
}

- (void)printPlatformVersionWarning
{
    if (!IsAtLeastiOSVersion(@"8.0")) {
        NSLog(@"CRITICAL: For NodeKit 4.0.0 and above, you will need to upgrade to at least iOS 8.0 or greater. Your current version of iOS is %@.",
...
-(NSString*)configFilePath{
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
        NSAssert(NO, @"ERROR: %@ does not exist. Please run nodekit-ios/bin/nodekit_plist_to_config_xml path/to/project.", path);
        return nil;
    }
...
- (NSURL*)errorURL
{
    ...
    id setting = [self.settings nodekitSettingForKey:@"ErrorUrl"];
...
- (void)viewDidLoad
{
    ...
    id backupWebStorage = [self.settings nodekitSettingForKey:@"BackupWebStorage"];
    if ([backupWebStorage isKindOfClass:[NSString class]]) {
        backupWebStorageType = backupWebStorage;
    }
    [self.settings setNodeKitSetting:backupWebStorageType forKey:@"BackupWebStorage"];
...
- (UIView*)newNodeKitViewWithFrame:(CGRect)bounds
{
    NSString* defaultWebViewEngineClass = [self.settings nodekitSettingForKey:@"NodeKitDefaultWebViewEngine"];
    NSString* webViewEngineClass = [self.settings nodekitSettingForKey:@"NodeKitWebViewEngine"];
...
- (NSString*)userAgent
{
    ...
    } else if ([self.settings nodekitSettingForKey:@"OverrideUserAgent"] != nil) {
        localBaseUserAgent = [self.settings nodekitSettingForKey:@"OverrideUserAgent"];
    } else {
        localBaseUserAgent = [CDVUserAgentUtil originalUserAgent];
    }
    NSString* appendUserAgent = [self.settings nodekitSettingForKey:@"AppendUserAgent"];
    if (appendUserAgent) {
...
- (void)createGapView
{
    ...
    UIView* view = [self newNodeKitViewWithFrame:webViewBounds];
...
/**
 Returns an instance of a NodeKitCommand object, based on its name.  If one exists already, it is returned.
 */
- (id)getCommandInstance:(NSString*)pluginName
{
    ...
    // "io.nodekit.Foo" and "io.nodekit.foo" - only the lower-cased entry will match
...
- (void)onAppWillResignActive:(NSNotification*)notification
{
    ...
    [self.commandDelegate evalJs:@"nodekit.fireDocumentEvent('resign');" scheduledOnRunLoop:NO];
}
...
- (void)onAppWillEnterForeground:(NSNotification*)notification
{
    ...
    [self.commandDelegate evalJs:@"nodekit.fireDocumentEvent('resume');"];
...
- (void)onAppDidBecomeActive:(NSNotification*)notification
{
    ...
    [self.commandDelegate evalJs:@"nodekit.fireDocumentEvent('active');"];
}
...
- (void)onAppDidEnterBackground:(NSNotification*)notification
{
    ...
    [self.commandDelegate evalJs:@"nodekit.fireDocumentEvent('pause', null, true);" scheduledOnRunLoop:NO];
}
