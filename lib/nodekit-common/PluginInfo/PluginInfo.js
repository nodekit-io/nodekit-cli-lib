...
var path = require('path')
  ...
  , NodeKitError = require('../NodeKitError/NodeKitError')
  ;
...
function PluginInfo(dirname) {
    ...
    if (!fs.existsSync(self.filepath)) {
        throw new NodeKitError('Cannot find plugin.xml for plugin "' + path.basename(dirname) + '". Please try adding it again.');
    }
    ...
    self.getKeywordsAndPlatforms = function () {
        ...
        return ret.concat('ecosystem:nodekit').concat(addNodeKit(self.getPlatformsArray()));
    };
}  // End of PluginInfo constructor.

// Helper function used to prefix every element of an array with nodekit-
// Useful when we want to modify platforms to be nodekit-platform
function addNodeKit(someArray) {
    var newArray = someArray.map(function(element) {
        return 'nodekit-' + element;
    });
    return newArray;
}
