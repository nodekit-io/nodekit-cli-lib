...
var NodeKitError = require('./NodeKitError/NodeKitError');
 ...
 *   instantiated directly, `NodeKitLogger.get()` method should be used instead
 *   to acquire logger instance
 */
function NodeKitLogger () {
    this.levels = {};
    this.colors = {};
 ...
 *
 * @return  {NodeKitLogger}  Logger instance
 */
NodeKitLogger.get = function () {
    return INSTANCE || (INSTANCE = new NodeKitLogger());
};

NodeKitLogger.VERBOSE = 'verbose';
NodeKitLogger.NORMAL = 'normal';
NodeKitLogger.WARN = 'warn';
NodeKitLogger.INFO = 'info';
NodeKitLogger.ERROR = 'error';
NodeKitLogger.RESULTS = 'results';
 ...
 *   `NodeKitLogger.NORMAL` level will be used.
 * @param   {String}  message   The message, that should be logged to process'
 *   stdio
 *
 * @return  {NodeKitLogger}     Current instance, to allow calls chaining.
 */
NodeKitLogger.prototype.log = function (logLevel, message) {
    ...
    if (message instanceof Error || logLevel === NodeKitLogger.ERROR) {
 ...
 * @return  {NodeKitLogger}     Current instance, to allow calls chaining.
 */
NodeKitLogger.prototype.addLevel = function (level, severity, color) {
 ...
 *   with this name, `NodeKitLogger.NORMAL` will be used.
 *
 ...
 * @return  {NodeKitLogger}     Current instance, to allow calls chaining.
 */
NodeKitLogger.prototype.setLevel = function (logLevel) {
    this.logLevel = this.levels[logLevel] ? logLevel : NodeKitLogger.NORMAL;
 ...
 * @return  {NodeKitLogger}     Current instance, to allow calls chaining.
 */
NodeKitLogger.prototype.adjustLevel = function (opts) {
 ...
 * @return  {NodeKitLogger}     Current instance, to allow calls chaining.
 */
NodeKitLogger.prototype.subscribe = function (eventEmitter) {
...
function formatError(error, isVerbose) {
    ...
    if (error instanceof NodeKitError) {
