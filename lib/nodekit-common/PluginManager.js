 ...
 *  * Generate metadata for plugin and plugin modules and save it to 'nodekit_plugins.js'
 *
...
PluginManager.prototype.doOperation = function (operation, plugin, options) {
    if (operation !== PluginManager.INSTALL && operation !== PluginManager.UNINSTALL)
        return Q.reject(new NodeKitError('The parameter is incorrect. The opeation must be either "add" or "remove"'));

    if (!plugin || plugin.constructor.name !== 'PluginInfo')
        return Q.reject(new NodeKitError('The parameter is incorrect. The first parameter should be a PluginInfo instance'));
    ...
    .then(function () {
        ...
        fs.writeFileSync(path.join(self.locations.www, 'nodekit_plugins.js'), metadata, 'utf-8');
        ...
        if (options.usePlatformWww) {
            fs.writeFileSync(path.join(self.locations.platformWww, 'nodekit_plugins.js'), metadata, 'utf-8');
        }
    });
};
