...
module.exports = function info() {
    ...
    var output = '';
    if (!projectRoot) {
        return Q.reject(new Error('Current working directory is not a NodeKit-based project.'));
    }
...
function getPlatforms(projectRoot) {
    var platforms = nodekit_util.listPlatforms(projectRoot);
    if (platforms.length) {
...
function listPlugins(projectRoot) {
    var pluginPath = path.join(projectRoot, 'plugins'),
        plugins    = nodekit_util.findPlugins(pluginPath);
