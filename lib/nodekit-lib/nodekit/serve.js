...
var nodekit_util = require('./util'),
    ...
    serve        = require('nodekit-cli-lib')['nodekit-serve'];
...
function handleRoot(request, response, next) {
    ...
    var config = new ConfigParser(nodekit_util.projectConfig(projectRoot));
...
module.exports = function server(port, opts) {
    ...
    return Q.promise(function(resolve) {
        projectRoot = nodekit_util.cdProjectRoot();

        var hooksRunner = new HooksRunner(projectRoot);
        hooksRunner.fire('before_serve', opts).then(function () {
            // Run a prepare first!
            return require('./nodekit').raw.prepare([]);
        }).then(function () {
            var server = serve();

            installedPlatforms = nodekit_util.listPlatforms(projectRoot);
