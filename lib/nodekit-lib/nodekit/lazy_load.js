...
exports.nodekit_git = nodekit_git;
exports.git_clone = git_clone_platform;
exports.nodekit_npm = nodekit_npm;
...
function based_on_config(project_root, platform, opts) {
    ...
    if (custom_path) {
        ...
        return module.exports.nodekit(platform, opts);
    }
}

// Returns a promise for the path to the lazy-loaded directory.
function nodekit(platform, opts) {
    platform = new Platform(platform);
    var use_git = platform.source === 'git';
    if ( use_git ) {
        return module.exports.nodekit_git(platform);
    } else {
        return module.exports.nodekit_npm(platform);
    }
}

function nodekit_git(platform) {
    var mixed_platforms = _.extend({}, platforms),
        plat;
    if (!(platform.name in platforms)) {
        return Q.reject(new Error('NodeKit library "' + platform.name + '" not recognized.'));
    }
...
function nodekit_npm(platform) {
    ...
    return util.getLatestMatchingNpmVersion(platform.packageName, platform.version).then(function (version) {
        var git_dload_dir = path.join(util.libDirectory, platdir, 'nodekit', version);
...
// npm cache add nodekit-android@3.5.0
...
function custom(platforms, platform) {
    ...
    if (!(platform in platforms)) {
        return Q.reject(new Error('NodeKit library "' + platform + '" not recognized.'));
    }
...
function git_clone_platform(git_url, branch) {
    // Create a tmp dir. Using /tmp is a problem because it's often on a different partition and sehll.mv()
    // fails in this case with "EXDEV, cross-device link not permitted".
    var tmp_subidr = 'tmp_nodekit_git_' + process.pid + '_' + (new Date()).valueOf();
