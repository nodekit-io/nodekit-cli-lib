...
function ios_parser(project) {
    ...
    try {
        ...
        if (!xcodeproj_dir) throw new NodeKitError('The provided path "' + project + '" is not a NodeKit iOS project.');
        ...
        this.nodekitproj = path.join(project, this.originalName);
    } catch(e) {
        throw new NodeKitError('The provided path "'+project+'" is not a NodeKit iOS project.');
    }
    this.path = unorm.nfd(project);
    this.pbxproj = path.join(this.xcodeproj, 'project.pbxproj');
    this.config_path = path.join(this.nodekitproj, 'config.xml');
}
...
ios_parser.prototype.update_from_config = function(config) {
    ...
    var plistFile = path.join(this.nodekitproj, this.originalName + '-Info.plist');
    ...
    var platformRoot = this.nodekitproj;
    var appRoot = util.isNodeKit(platformRoot);
    ...
    return this.update_build_settings(config).then(function() {
        ...
        shell.mv(path.join(parser.nodekitproj, parser.originalName + '-Info.plist'), path.join(parser.nodekitproj, name + '-Info.plist'));
        shell.mv(path.join(parser.nodekitproj, parser.originalName + '-Prefix.pch'), path.join(parser.nodekitproj, name + '-Prefix.pch'));
        ...
        shell.mv(parser.nodekitproj, path.join(parser.path, name));
...
ios_parser.prototype.nodekitjs_path = function(libDir) {
    var jsPath = path.join(libDir, 'NodeKitLib', 'nodekit.js');
    return path.resolve(jsPath);
};

ios_parser.prototype.nodekitjs_src_path = function(libDir) {
...
ios_parser.prototype.update_www = function() {
    ...
    // Copy over stock platform www assets (nodekit.js)
...
// Converts nodekit specific representation of target device to XCode value
