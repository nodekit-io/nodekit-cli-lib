...
var fs            = require('fs'),
    ...
    xml           = require('nodekit-cli-lib')['nodekit-common'].xmlHelpers,
    util          = require('../util'),
...
function amazon_fireos_parser(project) {
    if (!fs.existsSync(path.join(project, 'AndroidManifest.xml'))) {
        throw new NodeKitError('The provided path "' + project + '" is not an Android project.');
    }
...
amazon_fireos_parser.prototype.update_from_config = function(config) {
    ...
    var java_files = fs.readdirSync(orig_pkgDir).filter(function(f) {
      return f.indexOf('.svn') == -1 && f.indexOf('.java') >= 0 && fs.readFileSync(path.join(orig_pkgDir, f), 'utf-8').match(/extends\s+NodeKitActivity/);
    });
    if (java_files.length === 0) {
      throw new Error('No Java files found which extend NodeKitActivity.');
    } else if(java_files.length > 1) {
      events.emit('log', 'Multiple candidate Java files (.java files which extend NodeKitActivity) found. Guessing at the first one, ' + java_files[0]);
    }
...
amazon_fireos_parser.prototype.nodekitjs_path = function(libDir) {
    var jsPath = path.join(libDir, 'framework', 'assets', 'www', 'nodekit.js');
    return path.resolve(jsPath);
};

amazon_fireos_parser.prototype.nodekitjs_src_path = function(libDir) {
