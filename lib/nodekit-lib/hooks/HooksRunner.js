/**
 ...
 under the License.
 */
var nodekitUtil  = require('../nodekit/util'),
...
function HooksRunner(projectRoot) {
    var root = nodekitUtil.isNodeKit(projectRoot);
    if (!root) throw new NodeKitError('Not a NodeKit project ("' + projectRoot + '"), can\'t use hooks.');
...
HooksRunner.prototype.prepareOptions = function(opts) {
    ...
    opts.nodekit = opts.nodekit || {};
    opts.nodekit.platforms = opts.nodekit.platforms || opts.platforms || nodekitUtil.listPlatforms(opts.projectRoot);
    opts.nodekit.platforms = opts.nodekit.platforms.map(function(platform) { return platform.split('@')[0]; } );
    opts.nodekit.plugins = opts.nodekit.plugins || opts.plugins || nodekitUtil.findPlugins(path.join(opts.projectRoot, 'plugins'));

    try {
        opts.nodekit.version = opts.nodekit.version || require('../../../package').version;
    } catch(ex) {
...
function runScriptViaChildProcessSpawn(script, context) {
    ...
    execOpts.env = {};
    execOpts.env.NODEKIT_VERSION = require('../../../package').version;
    execOpts.env.NODEKIT_PLATFORMS = opts.platforms ? opts.platforms.join() : '';
    execOpts.env.NODEKIT_PLUGINS = opts.plugins ? opts.plugins.join() : '';
    execOpts.env.NODEKIT_HOOK = script.fullPath;
    execOpts.env.NODEKIT_CMDLINE = process.argv.join(' ');
