...
var shell   = require('shelljs'),
    ...
    PluginInfoProvider = require('nodekit-cli-lib')['nodekit-common'].PluginInfoProvider,
...
function fetchPlugin(plugin_src, plugins_dir, options) {
    ...
    if (uri.hash) {
        ...
        if (result) {
            ...
            if (options.subdir && options.subdir !== '.') {
                events.emit('warn', 'support for subdirectories is deprecated and will be removed in NodeKit@7');
                if (options.fetch) {
                    return Q.reject(new NodeKitError('--fetch does not support subdirectories'));
                }
            }
            ...
            //nodekit-fetch doesn't need to strip out git-ref
            if(!options.fetch) {
    ...
    return Q.when().then(function() {
        ...
        var plugin_dir = nodekitUtil.fixRelativePath(path.join(plugin_src, options.subdir));
        ...
        return Q.when().then(function() {
            ...
            if (pinfo) {
                ...
                return Q.reject(new NodeKitError(
            ...
            if (fs.existsSync(plugin_dir)) {
                ...
                if (alias && fs.existsSync(path.join(plugins_dir, alias))) {
                    ...
                    //use nodekit-fetch if --fetch was passed in
                    if(options.fetch) {
    ...
    if(!link && dest.indexOf(path.resolve(plugin_dir)+path.sep) === 0) {
        ...
        if(/^win/.test(process.platform)) {
            /*
                ...
                and to install the plugin via `nodekit plugin add ../`
            */
