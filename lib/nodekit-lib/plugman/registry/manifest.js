...
function validateName(name) {
    ...
    if (name.match(/io.nodekit\..*/) && whitelist.indexOf(name) === -1) {
        throw new Error('Invalid Plugin ID. The "io.nodekit" prefix is reserved for plugins provided directly by the NodeKit project.');
    }
...
function generatePackageJsonFromPluginXml(plugin_path) {
    return Q().then(function() {
        ...
        if(nodekit_name) package_json.nodekit_name = nodekit_name;
