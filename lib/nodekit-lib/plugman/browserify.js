...
function getPlatformVersion(cId, project_dir) {
    ...
    var versionPath = path.join(project_dir, '/nodekit/version');
...
module.exports = function doBrowserify (project, platformApi, pluginInfoProvider) {
    ...
    // Write this object into www/nodekit_plugins.json.
    ...
    .then(function(cId){
        ...
        libraryRelease
            .add(nodekit_plugins, {file: path.join(wwwDir, 'nodekit_plugins.js'), expose: 'nodekit/plugin_list'})
            .add(bootstrap)
            .transform(aliasify, {aliases: moduleAliases});

        var outReleaseFile = path.join(wwwDir, 'nodekit.js');
