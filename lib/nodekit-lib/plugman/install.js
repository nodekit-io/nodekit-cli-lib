...
var path = require('path'),
    fs   = require('fs'),
    action_stack = require('nodekit-cli-lib')['nodekit-common'].ActionStack,
    ...
    pluginSpec = require('../nodekit/plugin_spec_parser'),
    nodekitUtil = require('../nodekit/util');
...
module.exports = function installPlugin(platform, project_dir, id, plugins_dir, options) {
    ...
    plugins_dir = plugins_dir || path.join(project_dir, 'nodekit', 'plugins');
...
function getEngines(pluginInfo, platform, project_dir, plugin_dir){
    ...
    var nodekitEngineIndex, nodekitPlatformEngineIndex, theName, platformIndex, defaultPlatformIndex;
    ...
    engines.forEach(function(engine) {
        ...
        if (defaultEngines[theName]) {
            ...
            if(defaultPlatformIndex > -1 || defaultEngines[theName].platform === '*'){
                ...
                // set the indices so we can pop the nodekit engine when needed
                if(theName==='nodekit') nodekitEngineIndex = uncheckedEngines.length;
                if(theName==='nodekit-'+platform) nodekitPlatformEngineIndex = uncheckedEngines.length;
        ...
        // check for other engines
            ...
            if (typeof engine.platform === 'undefined' || typeof engine.scriptSrc === 'undefined') {
                throw new NodeKitError('warn', 'engine.platform or engine.scriptSrc is not defined in custom engine "' +
    ...
    // make sure we check for platform req's and not just nodekit reqs
    if(nodekitEngineIndex && nodekitPlatformEngineIndex) uncheckedEngines.pop(nodekitEngineIndex);
...
function runInstall(actions, platform, project_dir, plugin_dir, plugins_dir, options) {
    project_dir = nodekitUtil.convertToRealPathSafe(project_dir);
    plugin_dir = nodekitUtil.convertToRealPathSafe(plugin_dir);
    plugins_dir = nodekitUtil.convertToRealPathSafe(plugins_dir);
    ...
    return Q().then(function() {
        ...
        return Q(superspawn.maybeSpawn(path.join(project_dir, 'nodekit', 'version'), [], { chmod: true }));
...
function tryFetchDependency(dep, install, options) {
    ...
    if ( dep.url == '.' ) {
        ...
        if (fetchdata.source.type === 'local') {
            ...
            // $md5 = md5(uri)
            // Need a Hash(uri) --> $tmpDir/nodekit-fetch/git-hostname.com-$md5/
...
function handleInstall(actions, pluginInfo, platform, project_dir, plugins_dir, plugin_dir, filtered_variables, options) {
    ...
    .then (function(result) {
        ...
        if (platform == 'android' &&
            ...
            var buildModule;
            try {
                buildModule = require(path.join(project_dir, 'nodekit', 'lib', 'build'));
            } catch (e) {
