...
module.exports = {
    ...
    'framework':{ // CB-5238 custom frameworks only
        install:function(obj, plugin_dir, project_dir, plugin_id, options, project) {
            ...
            if (!custom) {
                ...
                // CoreLocation dependency removed in nodekit-ios@3.6.0.
                if (semver.lt(project.nodekitVersion, '3.6.0-dev')) {
    ...
    parseProjectFile:function(project_dir) {
        ...
        // we don't want NodeKitLib's xcode project
        ...
        if (!fs.existsSync(plist_file) || !fs.existsSync(config_file)) {
            throw new NodeKitError('could not find -Info.plist file, or config.xml file.');
        }
        ...
        var nodekitVersion = fs.readFileSync(path.join(project_dir, 'NodeKitLib', 'VERSION'), 'utf8').trim();
