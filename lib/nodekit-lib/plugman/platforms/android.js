...
var path = require('path')
   ...
   , xml_helpers = require('nodekit-cli-lib')['nodekit-common'].xmlHelpers
...
module.exports = {
    ...
    'source-file':{
        install:function(obj, plugin_dir, project_dir, plugin_id, options) {
            var src = obj.src;
            if (!src) {
                throw new NodeKitError('<source-file> element is missing "src" attribute for plugin: ' + plugin_id);
            }
            var targetDir = obj.targetDir;
            if (!targetDir) {
                throw new NodeKitError('<source-file> element is missing "target-dir" attribute for plugin: ' + plugin_id);
            }
    ...
    'framework': {
        install:function(obj, plugin_dir, project_dir, plugin_id, options) {
            ...
            if (!src) throw new NodeKitError('src not specified in <framework> for plugin: ' + plugin_id);
